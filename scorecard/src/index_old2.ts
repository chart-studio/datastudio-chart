...
const drawViz = (vizData: ObjectFormat) => {
  ...
  const datasetCompare = hasCompareTables
    ? transformData(vizData.tables.COMPARISON)
    : null;
  ...
  listVarsMetric.map((metric, i) => {
    ...
    const card = wrapper
      ...
      .attr(
        "style",
        `position: relative; width:${cardSize.w}px; height: ${
          cardSize.h
...
function getStyle(vizData: ObjectFormat) {
  let fontFamily = styleVal(vizData, "fontFamily") || "Roboto";
