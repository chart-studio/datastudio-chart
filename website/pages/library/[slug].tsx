...
const Doc = ({ doc, locale }: { doc: MDXDoc; locale: locale }) => {
  ...
  return (
    <>
      <SeoHome title={`${doc.meta.title} | ${t.navbar.link3}`} />
      <GridDocs>
        <GraphContent>
          <GraphContentWrapper>
            ...
            <LinkHold
              ...
              testLink={
                user?.status_subsc
                  ? user?.subsc.find(
                      graph => graph.graphname === doc.meta.chartInfo.cloudName
                    )?.link || ""
                  : user?.trygraph.find(
                      graph => graph.graphname === doc.meta.chartInfo.cloudName
                    )?.link || ""
        ...
        <DataContent>
          ...
          <h6>{t.docPage.titreSubData2}</h6>
          <table className="tg">
            <thead>
              <tr>
                {Object.keys(doc.meta.chartInfo.example[0]).map((label, i) => (
                  <th className="tg-0lax" key={i}>
                    {label}
                  </th>
                ))}
            ...
            <tbody>
              {doc.meta.chartInfo.example.map((row: any, i: number) => {
                return (
                  <tr key={i}>
                    {Object.keys(row).map((val: string, d: number) => (
                      <td
